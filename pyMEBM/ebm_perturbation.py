...
class EBMPert(EBM):
    ...
    **Initialization parameters** \n
    ...
    def __init__(
        self, ocn_heat_uptake=False, forcing="flat", feedbacks="flat", **kwargs
    ):
    ...
    def _load_ctrl_data(self):
        ...
        T_ctrl = griddata(np.sin(np.deg2rad(lat)), T_ctrl, self.x, method="linear")
        ...
        q_ctrl = (
            ...
            * self.param["relhum"]
            / self.const["psfc"]
            * self.const["e0"]
            * np.exp(self.const["a"] * (T_ctrl) / (self.const["b"] + (T_ctrl)))
        )
        theta_e_ctrl = (
            1
            / self.const["cp"]
    ...
    def _load_forcing_feedbacks(self):
        ...
        if self.forcing == "flat":
            ...
            R_frc = griddata(CMIP5_lat, np.mean(frc, axis=1), self.phi, method="linear")
        ...
        if self.feedbacks == "flat":
            ...
            B = -griddata(
                CMIP5_lat, np.mean(CMIP5_lambda, axis=1), self.phi, method="linear"
            )  # taking average over CMIP5 models
    ...
    def step(self):
        ...
        self.variables["q"] = q0 * np.exp(
            self.const["a"]
            * (self.variables["T"] + self.variables["T_ctrl"])
            / (self.const["b"] + self.variables["T"] + self.variables["T_ctrl"])
        )

        self.variables["theta_e"] = (
            1
            / self.const["cp"]
            * (
                self.const["cp"]
                * (self.variables["T"] + self.variables["T_ctrl"] + 273.15)
        ...
        Src = self.param["A0"] + self.param["B0"] * (self.variables["T"])
